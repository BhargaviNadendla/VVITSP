...
public class DeleteSubjectBean {
    ...
    public String deleteSubjectAction() {
        ...
        Long subjectId = ((Number) currentRow.getAttribute("SubjectId")).longValue();        
        ...
        try {
            ...
            ViewCriteria criteria = vo.getViewCriteriaManager().getViewCriteria("findBySubjectIdVC");
            VariableValueManager manager = vo.ensureVariableManager();
            manager.setVariableValue("bindSubjectId", subjectId);                
            vo.applyViewCriteria(criteria);
            vo.executeQuery();
            studentRow = vo.first();
            studentRow.setAttribute("Active", 0);            
            
            ViewObject bssvo = commonAM.getBranchSemesterSubjectVO1();
            Row besRow = null;
            bssvo.setWhereClause(null);
            bssvo.setWhereClauseParams(null);
            bssvo.applyViewCriteria(null);
            criteria = bssvo.getViewCriteriaManager().getViewCriteria("findByBranchSemesterSubjectVC");
            manager = bssvo.ensureVariableManager();
            manager.setVariableValue("bindSubjectId", subjectId);
            manager.setVariableValue("bindBranchId", ((Number) currentRow.getAttribute("BranchId")).longValue());
            manager.setVariableValue("bindSemesterId", ((Number) currentRow.getAttribute("SemesterId")).longValue());
            bssvo.applyViewCriteria(criteria);
            bssvo.executeQuery();
            besRow = bssvo.first();
